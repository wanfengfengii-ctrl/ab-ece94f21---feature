...
- 可选**连续掩模岛**：启用后每张实际使用的掩模中，所有片段必须仅沿录入的无向邻接边互达；
  该约束与冲突、缝合、规范化、唯一性在同一次精确求解中同时成立（不是先求解再过滤）
- 输入错误逐项展示并清除旧结果；无解区分为冲突图不可三染色与**连续性阻断**
- 逐条列出被切开的缝合边，并按掩模展示足以复核连通性的采用邻接边与片段覆盖
...
见证/切开边集合）、无解情形的区分、输入错误逐项返回，以及连续掩模岛的三种
情形：连续可行（含与暴力枚举 oracle 的连通性对照）、连续性阻断、关闭模式回归
（请求/响应/无解语义保持原样）和邻接输入逐项拒绝。
...
- `contiguous`：`true` 时启用连续掩模岛（可缺省，默认 `false`，此时其余行为与旧版完全一致）
- `adjacency_edges`：启用时必填的无向邻接边二元数组列表（至少一条），作为同掩模连通的唯一依据
...
- 关闭模式下响应字段与旧版完全一致（不含 `connectivity`）
- 无解时返回 `{"status": "infeasible"}`（HTTP 200）；连续模式下附带
  `reason`：`"connectivity_blocked"` 表示原模型可解而连续性使其无解，
  `"conflict_graph"` 表示冲突图本身不可三染色

连续模式响应在上述字段基础上，为最优方案与见证各附带一份 `connectivity`。
上面的连续模式请求返回：
...
```json
{
  ...
  "cut_stitches": [{"pair": [3, 4], "weight": 1}],
  "connectivity": {
    "enabled": true,
    "masks": [
      {"mask": 0, "fragments": [1, 3], "adjacency_edges": [[1, 3]]},
      {"mask": 1, "fragments": [2, 4], "adjacency_edges": [[2, 4]]}
    ]
  },
  "witness": { "...": "另一份最优方案，同样带 connectivity" }
}
```

- `masks` 只列出实际使用的掩模；`fragments` 是该掩模的全部覆盖片段，
  `adjacency_edges` 是承载连通流的采用邻接边（沿其可从任一覆盖片段到达其余片段）
...
- 边的两个端点都必须出现在片段列表中
- 连续掩模岛（`contiguous` 必须是布尔值）启用时：
  - `adjacency_edges` 至少包含一条边，每条为两个整数端点的二元数组
  - 逐项拒绝未知片段端点、自环、重复无向点对（含顺序互换）、格式错误
...
- 连续掩模岛（启用时）：为每个掩模 k 建模一棵以“首个同色片段”为根的
  单商品流——每个非根同色片段净流入 1，根流出 `同色数 − 1`；
  弧流量被两端颜色变量约束（`f_uv ≤ (n−1)·x[u][k]` 且 `≤ (n−1)·x[v][k]`），
  因此流只能经过两端同色的邻接边。流变量全为连续变量，颜色一旦取整即为
  纯网络 LP。连通约束与冲突/缝合/规范化在同一模型中，绝不先求旧方案再过滤
- 求得最优值后固定最优面，按 8 个片段为一组（三进制加权）最小化得到
  字典序最小方案（连续模式附带完整可行的连通流 warm start）；
  再加排除该方案的割平面重解，判定唯一性并取回不同见证
- 连续模式无解时重解一次不含连通约束的基础模型，以区分
  `connectivity_blocked`（连续性阻断）与 `conflict_graph`（冲突图不可三染色）
